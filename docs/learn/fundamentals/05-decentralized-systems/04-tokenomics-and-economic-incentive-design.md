...
In decentralized protocols, there is no corporate entity, no human resources department, and no physical office.
A protocol is an open-source collection of smart contracts deployed to an immutable blockchain.
...
1. **Network Fuel (Gas Tokens):** The base-layer asset required to pay validators for hardware execution (such as BTC, ETH, or SOL).
   Demand scales directly with network transaction volume.
2. **Governance Tokens:** Grants holders voting rights on protocol risk parameters, interest rate curves, code upgrades, and treasury grant disbursements (such as UNI or COMP).
...
- **Cliff Period (such as 1 Year):** A mandatory freeze window where zero tokens are unlocked. If a founder leaves the team during the cliff, they forfeit 100 percent of their allocation.
...
flowchart TD
    ...
    CapitalInflow --> DumpTokens["Yield Farmers (Bob) Immediately Dump Earned Tokens on Market for USDC/ETH"]
...
- **Irrevocable Lock:** Alice locks CRV tokens in a smart contract for anywhere from **1 week up to 4 years**.
...
- **Linear Decay:** Locking 1 CRV for 4 years grants **1.0 veCRV**.
  ...
  As time passes toward the unlock date, Alice's veCRV balance decays linearly to zero.
...
- Dedicated **Bribe Markets** (such as Votium) emerged where protocols openly paid millions of dollars in weekly cash bribes directly to veCRV voters to direct emissions to their pools.

The veToken model transformed governance from a speculative token into an active, cash-flow-generating economic asset, permanently aligning long-term token holders with protocol survival.

## The Next Question: Who Governs the Code?

We have explored how tokens align economic incentives, distribute emissions, and coordinate capital across time.
However, this leads directly into the ultimate institutional question:
Who actually controls the smart contract addresses that upgrade code, manage multi-million-dollar treasuries, and adjust protocol parameters?

In traditional corporations, boards of directors and executives make closed-door decisions.
In decentralized networks, governance is coordinated through **Decentralized Autonomous Organizations (DAOs)**.

How do on-chain proposals, timelocks, and Governor contracts automate democratic decisions?
How do multi-sig councils and delegation frameworks prevent voter apathy?
And what real-world governance attacks (such as flash loan governance takeovers) threaten protocol treasuries?
To investigate sovereign algorithmic governance, we step into **Decentralized Autonomous Organizations (DAOs)**.
